# Graph Report - tdd-agentic  (2026-06-03)

## Corpus Check
- 121 files · ~50,359 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1251 nodes · 2554 edges · 87 communities (79 shown, 8 thin omitted)
...
- `Agent Context and Workflow` --references--> `CRAG Retrieval Pipeline`  [EXTRACTED]
  docs/agent-context-and-workflow.md → README.md
- `Agent Context and Workflow` --references--> `Human-in-the-Loop Interface`  [EXTRACTED]
  docs/agent-context-and-workflow.md → README.md
...
Cohesion: 0.06
Nodes (132): Event, AgentRole, AnswerQuestion, AsyncSession, str, SubtaskCreate, SubtaskStatus, SubtaskUpdate (+124 more)
...
Cohesion: 0.05
Nodes (54): build_root_graph(), Root LangGraph orchestration graph.  The Project Manager acts as the supervisor;, Conditional edge: dispatch from the project manager based on its decision., Compile the full multi-agent orchestration graph., _route_from_pm(), build_specialist_subgraph(), Build and compile a tool-using specialist subgraph., Build and compile a tool-using specialist subgraph. (+46 more)
...
Cohesion: 0.14
Nodes (22): str, int, Path, str, create_skill(), Researcher subgraph — web search, doc writing, RAG ingestion, skill creation., Create or update a Skill (a focused capability brief) and assign it to roles., fs_delete() (+14 more)
...
Cohesion: 0.27
Nodes (13): _cache_bytes_total(), _estimate_bytes(), _evict_cache_if_needed(), _fetch_checkpoints(), get_checkpoints_list(), _lock(), In-memory TTL + singleflight for checkpoint list API.  Repeated GET /agents/chec, Approximate in-memory size of a JSON-like structure.      We avoid json.dumps he (+5 more)
...
Cohesion: 0.13
Nodes (18): _build_serde(), close_pool(), get_checkpointer(), get_pool(), LangGraph checkpointer setup using AsyncPostgresSaver.  We expose a context-mana, Return a serializer that knows about our application state types.      ``allowed, Async context manager that yields an initialized AsyncPostgresSaver.      On fir, create_app() (+10 more)
...
Cohesion: 0.31
Nodes (12): describe_llm_slug(), log_llm_invoke_exception_context(), log_llm_invoke_start(), log_rag_crag_llm_targets(), Structured logging around LLM HTTP calls — see where traffic is sent., Return (provider, model_name, human_readable_endpoint_hint)., Call from inside an ``except`` block — attaches stack trace via ``exception()``., Call from inside an ``except`` block — attaches stack trace via ``exception()``. (+4 more)
...
Cohesion: 0.15
Nodes (17): bool, Ticket, str, _advance_in_review_to_todo(), Project Manager — supervisor agent.  Acts as the routing decision-maker. Holds t, Conservative readiness check for moving IN_REVIEW -> TODO.      User expectation, Conservative readiness check for moving IN_REVIEW -> TODO.      User expectation, Auto-advance tickets from IN_REVIEW to TODO when they are ready.      Returns th (+9 more)
...
## Knowledge Gaps
- **168 isolated node(s):** `nextConfig`, `config`, `name`, `version`, `private` (+163 more)
...
- **Why does `get_settings()` connect `LLM Routing & RAG Pipeline` to `Event Bus & Ticket Service`, `Agent Graph & Specialist Subgraphs`, `Message Reducer & Common Utils`, `Agent Logs & EventBus Persistence`, `Researcher Subgraph & Code Tools`, `Community 74`, `Skills Registry System`, `Agent API Routes & Checkpoint Cache`, `DB Session & Ticket API Routes`, `Community 79`, `Observability & Stream Helpers`, `Checkpoint List Cache`, `Community 81`, `Community 19`, `Community 53`, `Community 86`, `Community 22`, `Community 61`?**
  _High betweenness centrality (0.187) - this node is a cross-community bridge._
- **Why does `SubtaskStatus` connect `Event Bus & Ticket Service` to `Ticket Models & Schemas`, `Agent Graph & Specialist Subgraphs`, `Project Manager & Routing`, `Community 69`, `Community 77`, `Community 20`?**
