...
## Corpus Check
- 119 files · ~48,903 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1207 nodes · 2493 edges · 79 communities (71 shown, 8 thin omitted)
...
Cohesion: 0.06
Nodes (67): close_pool(), describe_llm_slug(), log_llm_invoke_exception_context(), log_llm_invoke_start(), log_rag_crag_llm_targets(), Structured logging around LLM HTTP calls — see where traffic is sent., Return (provider, model_name, human_readable_endpoint_hint)., Call from inside an ``except`` block — attaches stack trace via ``exception()``. (+59 more)

### Community 2 - "Event Bus & Ticket Service"
Cohesion: 0.07
Nodes (120): Event, SystemState, AgentRole, AnswerQuestion, SubtaskCreate, SubtaskStatus, SubtaskUpdate, TicketCreate (+112 more)
...
Cohesion: 0.05
Nodes (53): build_root_graph(), Root LangGraph orchestration graph.  The Project Manager acts as the supervisor;, Conditional edge: dispatch from the project manager based on its decision., Compile the full multi-agent orchestration graph., _route_from_pm(), build_specialist_subgraph(), Build and compile a tool-using specialist subgraph., Build and compile a tool-using specialist subgraph. (+45 more)
...
Cohesion: 0.06
Nodes (56): int, Any, int, str, Any, float, int, str (+48 more)
...
Cohesion: 0.07
Nodes (48): _build_specialist_input(), _last_text(), _latest_subtask_id_from_tools(), Classify the turn's verification signal from ``run_tests`` results.      Returns, IDs of subtasks the agent flipped to ``done`` via update_subtask_status., Force subtasks back to ``blocked`` (used when a completion gate fails)., Best-effort textual summary of the final AIMessage in ``messages``., Assemble the minimal message list a specialist needs to act.      Strategy: (+40 more)
...
Cohesion: 0.14
Nodes (18): get_cached_role_base(), Return the static base for a given role (cached once).      Only called at app s, Return the static base for a given role (cached once).      Only called at app s, Return the static base for a given role (cached once).      Only called at app s, Return the static base for a given role (cached once).      Only called at app s, Tests for prompt caching utility., get_cached_role_base raises ValueError for unknown roles., Calling get_cached_role_base multiple times returns the same object. (+10 more)
...
- **Why does `get_settings()` connect `LLM Routing & RAG Pipeline` to `Event Bus & Ticket Service`, `Agent Graph & Specialist Subgraphs`, `Message Reducer & Common Utils`, `Agent Logs & EventBus Persistence`, `Researcher Subgraph & Code Tools`, `Community 74`, `Agent API Routes & Checkpoint Cache`, `Skills Registry System`, `DB Session & Ticket API Routes`, `Observability & Stream Helpers`, `Checkpoint List Cache`, `Community 22`?**
  _High betweenness centrality (0.186) - this node is a cross-community bridge._
- **Why does `SubtaskStatus` connect `Event Bus & Ticket Service` to `Ticket Models & Schemas`, `LLM Routing & RAG Pipeline`, `Agent Graph & Specialist Subgraphs`, `Community 69`, `Project Manager & Routing`, `Agent Logs & EventBus Persistence`, `Community 20`?**
